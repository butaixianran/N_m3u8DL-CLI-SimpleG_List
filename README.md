...
# 更新Log
* v1.3.0 添加"置顶"按钮，把当前选择的任务，移动到队列最前面
* v1.2.0 队列的保存文件，改为json格式，以避免类序列化导致的版本问题。修复多出遗留问题。包括：下载完成后队列状态没有重置的bug；修改协议注册注销提示，只有在不是管理员模式才提示要求管理员权限；增加更多下载失败的log解析，以避免把下载失败当作成功，还在继续队列。
